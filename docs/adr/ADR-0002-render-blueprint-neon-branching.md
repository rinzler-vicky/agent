...
4. **`DATABASE_URL` stays `sync: false`** on the service — set once in the Render dashboard for the base service (pointing at the Neon project's default branch, named `production` in Console-created projects and `main` in API/CLI-created projects), and for previews it is set by the workflow after Render creates the preview service.
5. **`.github/workflows/pr-preview.yml` is slimmed** to: create a Neon branch named `preview/pr-<num>` (copy-on-write clone of the project's default branch), with an `expires_at` 14 days out as a safety net; discover the matching Render preview service via the Render API, PUT the Neon branch's connection string as `DATABASE_URL`, trigger a fresh deploy, wait for `/v1/health` to return 200, post the preview URL (plus a Neon block: branch / parent / expiry / console link) to the PR. A `neondatabase/schema-diff-action@v1` step (with `continue-on-error: true`) posts a separate Neon-authored comment when schemas differ. The teardown job deletes the Neon branch.
...
- Every preview has full schema, RLS policies, and `pgvector`/`uuid-ossp` extensions — inherited from the Neon default branch (`production` in this project) via copy-on-write. Phase 1's tenant-isolation guarantees apply to previews on day one.
...
- `NEON_API_KEY` granted write access to the Neon project. Compromise of the GitHub repo's Actions environment would let an attacker create/delete branches. Mitigated by scope (key has no access to data on the default branch beyond cloning, which is already part of the design) and by Neon's auto-rotation of integration-managed keys.
...
- Preview Neon branches are isolated logical databases with their own data, but they live in the same Neon project as the default branch (`production`). Anyone with `NEON_API_KEY` can read or delete any branch (including the default). Treat the Neon API key with the same care as `RENDER_API_KEY`.
...
- First-time setup checklist (also reproduced in `docs/PR_PREVIEWS.md`):
  ...
  2. Run `node backend/scripts/migrate.js` against the Neon default branch's (`production`) connection string to apply migrations `001..005`.
...
- Free-tier accounting: Neon Free plan allows 3 root branches per project. All preview branches are children of the default branch (`production`) and consume the child-branch allowance (currently ~10). For more concurrent PRs, upgrade to Launch plan or shorten `expires_at` on branches.
...
3. Confirm `SELECT count(*) FROM tenants;` on the preview's Neon branch succeeds (schema inheritance from the default branch, `production`).
