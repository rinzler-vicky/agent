...
export class AgentPreviewSpawnerService implements OnModuleInit {
    ...
    try {
      ...
      const service = await this.render.createService({
        ...
        envVars: [
          ...
          // JWT_SECRET will be regenerated by the service on first boot if
          // not provided; for an agent preview that's fine — no client logs in.
        ],
      });
  ...
  private async complete(
    id: string,
    tenantId: string,
    fields: { renderServiceId: string; neonBranchName: string; previewUrl: string },
  ): Promise<void> {
    const client = await this.pool.connect();
    try {
      await client.query('BEGIN');
      ...
      await client.query(
        `UPDATE preview_environments
            SET status = 'ready',
                ...
                updated_at = now()
          WHERE id = $1`,
        [id, fields.renderServiceId, fields.neonBranchName, fields.previewUrl],
      );
      await client.query('COMMIT');
    } catch (err) {
      await client.query('ROLLBACK').catch(() => undefined);
