...
* **Phase 2.5b — n8n preview surface REVERTED (2026-05-19)**: the original 2.5b design (per-PR `agent-n8n` Render service via Blueprint, paths-filter-gated wiring, single-process mode) was implemented but reverted before merge. Two empirical failures drove the revert: (1) Blueprint adoption immediately spawned a main-branch `agent-n8n` service that crash-looped (missing `DB_POSTGRESDB_*` env vars, which are `sync: false` so they don't propagate to the main service; production n8n was always served by docker-compose anyway) and billed real money until manually deleted; (2) after Blueprint re-adoption with the fixed `runtime: image` block, Render did NOT spawn a per-PR `agent-n8n` preview for the open PR — neither a `pull_request: synchronize` event nor manual re-adoption produced one. Render's docs are silent on preview-spawn behavior for `runtime: image` services (verified May 2026: blueprint-spec, preview-environments, list-services). Deferred to a future phase with a different mechanism. **What survives the revert**: paths-filter remains in `pr-preview.yml` (gating the smoke-test job), the `preview_environments` table + RLS + SECURITY DEFINER TTL helper (migration 014), the agent-spawner backend module (Render REST API path, backend-only — does not depend on Blueprint), and all 234 unit tests.
